...
export async function generateMetadata({
  ...
  const title = `${service.title} в Ростове-на-Дону — под ключ`;
  const description =
    service.shortDescription +
    ` Ростов-на-Дону и Ростовская область. Монтаж водоочистки под ключ. ${business.brandName}.`;
...
const DEFAULT_VARS = {
  ...
  cityNameGenitive: "Ростова-на-Дону",
...
export default async function ServicePage({
  ...
  const vars: typeof DEFAULT_VARS = {
    ...DEFAULT_VARS,
    serviceTitle: service.title,
    serviceShortDescription: service.shortDescription,
  };

  const seedKey = serviceSlug;
  ...
  const cityLinks = CITY_SLUGS.slice(0, 10).map((citySlug) => ({
    label: `${service.title} в ${cities[citySlug].name}`,
  ...
  return (
    <div className="bg-slate-50 min-h-screen">
      ...
      <ServiceHero
        title={`${service.title} в Ростове-на-Дону и области`}
        lead={`${heroLead} ${leadParagraph}`}
        imageSrc={getServiceHeroImage(serviceSlug)}
        imageAlt={`${service.title} — Ростов-на-Дону`}
      />
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl pb-10 sm:pb-16">
            ...
            <div className="p-4 sm:p-6">
              <div className="space-y-4 text-slate-700 text-sm sm:text-base leading-relaxed">
                {splitParagraphs(whenNeeded).map((para, i) => (
                  <p key={i} className="pl-0 sm:pl-0">{para}</p>
                ))}
              </div>
            </div>
          </div>
        ...
        <Problems />
        <Mounting formComment={`Рассчитать стоимость (блок Монтаж под ключ) — ${service.title}`} />
        <HowWeWork />
        <PricesBlock
          lead={pricesLead}
          pricesHref="/prices"
          ...
          items={priceItems}
        />
        <Quiz commentPrefix={`Интересует: ${service.title}`} />
        <WhyUs />
        <Geography />
        ...
        <SeeAlso title="Услуга в других городах области" links={cityLinks} />
          ...
          <div className="bg-white rounded-xl sm:rounded-2xl border border-slate-200 shadow-sm overflow-hidden transition-shadow duration-300 hover:shadow-md">
            ...
            <div className="p-4 sm:p-6 md:p-8">
              <p className="text-slate-600 mb-6">
                Оставьте имя и телефон — перезвоним и подготовим расчёт под ваш объект.
              </p>
              <div className="max-w-md">
                <ContactForm comment={`Интересует: ${service.title}`} />
              </div>
            </div>
          </div>
        </section>
