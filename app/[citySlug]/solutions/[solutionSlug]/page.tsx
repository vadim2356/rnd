...
import { getSolutionBySlug, SOLUTION_SLUGS, solutions } from "@/data/solutions";
import { business } from "@/data/business";
import { getPricesLead, getPricesSystemTypes } from "@/lib/content-generator";
...
export async function generateStaticParams() {
  const params: { citySlug: string; solutionSlug: string }[] = [];
  for (const citySlug of CITY_SLUGS) {
    for (const solutionSlug of SOLUTION_SLUGS) {
      params.push({ citySlug, solutionSlug });
    }
  }
  return params;
}
...
export async function generateMetadata({
  ...
  params: Promise<{ citySlug: string; solutionSlug: string }>;
}): Promise<Metadata> {
  const { citySlug, solutionSlug } = await params;
  ...
  const title = `${solution.metaTitle} в ${city.namePrepositional} — под ключ`;
  const description = `${solution.description.slice(0, 120)} ${city.name} и ${city.region}. ${business.brandName}.`;
  const path = `/${citySlug}/solutions/${solutionSlug}`;
...
export default async function CitySolutionPage({
  ...
  const breadcrumbs = [
    ...
    { name: solution.title, url: `/${citySlug}/solutions/${solutionSlug}` },
  ];

  const otherSolutions = SOLUTION_SLUGS.filter((s) => s !== solutionSlug).slice(0, 5);
  const solutionLinks = otherSolutions.map((slug) => ({
    label: `${solutions[slug].title} в ${city.name}`,
    href: `/${citySlug}/solutions/${slug}`,
  }));

  const serviceJsonLd = getServiceJsonLdDynamic(
    `${solution.title} в ${city.namePrepositional}`,
    ...
    <div className="bg-slate-50 min-h-screen">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-12 items-start">
            <div className="lg:col-span-3">
              <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold text-slate-900 leading-tight mb-6">
                {solution.title} в {city.namePrepositional}{citySlug === "rostov-na-donu" ? " и области" : ""}
              </h1>
            ...
            <div className="lg:col-span-2 relative aspect-[4/3] rounded-2xl overflow-hidden border border-slate-200 shadow-md bg-slate-100">
              <Image
                ...
                alt={`${solution.title} в ${city.namePrepositional}`}
                fill
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl pb-16">
        ...
        <Mounting
          ...
          formComment={`Рассчитать стоимость (решение) — ${city.name}, ${solution.title}`}
        />
        <HowWeWork />
        <PricesBlock
          ...
          items={priceItems}
        />
        <Quiz commentPrefix={`Интересует: ${solution.title} в ${city.namePrepositional}`} />
        <WhyUs />
        <Geography />

        <InstallationPhotos id="installations" embedded />

        <div className="mt-10">
          <FAQ />
        </div>

        <SeeAlso title="Другие решения в этом городе" links={solutionLinks} />
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden transition-shadow duration-300 hover:shadow-md">
            ...
            <div className="p-6 md:p-8">
              <p className="text-slate-600 mb-6">
                Оставьте заявку на {solution.titleShort} в {city.namePrepositional} — перезвоним и подготовим расчёт.
              </p>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-5xl">
                ...
                <div className="max-w-md">
                  <ContactForm comment={`Интересует: ${solution.title} в ${city.namePrepositional}`} />
                </div>
              </div>
            </div>
          </div>
